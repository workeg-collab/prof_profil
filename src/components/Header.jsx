...
import { Phone, MessageCircle, Menu, X, ShieldCheck, ChevronLeft } from 'lucide-react';
...
export default function Header() {
  ...
  const whatsappLink = "https://wa.me/201142466903?text=" + encodeURIComponent("مرحباً شركة المحترف للتجارة والتوريدات، أود الاستفسار عن الخدمات والأنظمة المتاحة لدكم");
    ...
    <header className={`fixed top-0 right-0 left-0 z-50 transition-all duration-300 ${
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 flex items-center justify-between">
        ...
        <a href="#hero" className="flex items-center gap-3 group">
          <img 
            src="/images/logo.jpg" 
            alt="شعار شركة المحترف للتجارة والتوريدات" 
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-500 font-medium">Professional Trading & Supplies</p>
          </div>
        </a>

        {/* Desktop Navigation Links */}
        <nav className="hidden lg:flex items-center gap-6 text-sm font-bold text-slate-700">
          <a href="#hero" className="hover:text-amber-600 transition-colors">الرئيسية</a>
          <a href="#about" className="hover:text-amber-600 transition-colors">عن الشركة</a>
          <a href="#policies" className="hover:text-amber-600 transition-colors">سياسة الرضا والتطوير</a>
          <a href="#services" className="hover:text-amber-600 transition-colors">خدماتنا وأنظمتنا</a>
          <a href="#clients" className="hover:text-amber-600 transition-colors">قائمة العملاء</a>
          <a href="#quote" className="hover:text-amber-600 transition-colors">طلب عرض سعر</a>
          <a href="#contact" className="hover:text-amber-600 transition-colors">تواصل معنا</a>
        </nav>
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden bg-white border-b border-slate-200 px-6 py-5 space-y-3 text-sm font-bold text-slate-800 shadow-xl">
          <a 
            ...
            className="block py-2 border-b border-slate-100 hover:text-amber-600"
          >
            الرئيسية
          </a>
          <a 
          ...
          <a 
            href="#contact" 
            onClick={() => setMobileMenuOpen(false)} 
            className="block py-2 text-amber-600 font-extrabold"
          >
            تواصل معنا
          </a>
