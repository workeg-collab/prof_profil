...
export default function Hero() {
  ...
  const whatsappLink = "https://wa.me/201142466903?text=" + encodeURIComponent(
    isRtl
      ? "مرحباً شركة المحترف للتجارة والتوريدات، أود الاستفسار عن أنظمة المراقبة والحلول التكنولوجية"
      : "Hello Professional Trading & Supplies, I would like to inquire about surveillance systems and technology solutions"
  );
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <p className="text-slate-700 text-base md:text-lg font-medium leading-relaxed max-w-2xl">
              {isRtl ? (
                ...
                <>
                  An innovative & progressive enterprise specializing in the design, distribution, installation, and maintenance of <strong className="text-slate-900">Surveillance Systems</strong>, <strong className="text-slate-900">Data Communication Networks</strong>, and integrated IT infrastructure solutions.
                </>
              )}
            </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2 text-slate-700 text-xs md:text-sm font-bold">
              <div className="flex items-center gap-2.5 bg-white/90 p-3 rounded-2xl border border-slate-200 shadow-sm">
                ...
                <span>{isRtl ? 'أنظمة مراقبة وكاميرات فائقة الدقة' : 'Ultra HD CCTV & IP Surveillance'}</span>
              </div>
              <div className="flex items-center gap-2.5 bg-white/90 p-3 rounded-2xl border border-slate-200 shadow-sm">
                <Network className="w-5 h-5 text-amber-600 shrink-0" />
                <span>{isRtl ? 'حلول البنية التحتية والشبكات' : 'Network & Data Infrastructure'}</span>
              </div>
              <div className="flex items-center gap-2.5 bg-white/90 p-3 rounded-2xl border border-slate-200 shadow-sm">
                <ShieldCheck className="w-5 h-5 text-blue-600 shrink-0" />
                <span>{isRtl ? 'أنظمة التحكم بالدخول والإنذار المبكر' : 'Access Control & Early Warning'}</span>
              </div>
              <div className="flex items-center gap-2.5 bg-white/90 p-3 rounded-2xl border border-slate-200 shadow-sm">
                <Award className="w-5 h-5 text-amber-600 shrink-0" />
                <span>{isRtl ? 'قائمة عريقة من العملاء الحكوميين والخاصين' : 'Prestigious Govt & Private Portfolio'}</span>
              </div>
            </div>
            ...
            <div className="pt-4 flex flex-wrap items-center gap-4">
              ...
              >
                ...
                <span>{isRtl ? 'استشارة فنية وعرض سعر فوري' : 'Get Instant Technical Consultation'}</span>
              </a>
              ...
              >
                <span>{isRtl ? 'استكشف كافة الخدمات (12 خدمة)' : 'Explore All 12 Services'}</span>
                <ArrowLeft className={`w-4 h-4 text-amber-600 ${!isRtl ? 'rotate-180' : ''}`} />
              </a>
            </div>
            ...
            <div className="relative rounded-3xl overflow-hidden shadow-2xl border-4 border-white bg-white">
              ...
              <div className="absolute bottom-4 right-4 left-4 bg-white/95 backdrop-blur-md p-4 rounded-2xl border border-slate-200 shadow-xl flex items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="font-extrabold text-slate-900 text-sm">{isRtl ? 'عام التأسيس والبداية' : 'Established in 2007'}</h4>
                    <p className="text-xs text-slate-500">{isRtl ? 'خبرة متواصلة وخدمات ما بعد البيع' : 'Continuous Expertise & Post-Sales Support'}</p>
                  </div>
                </div>
                <div className="bg-emerald-100 text-emerald-800 text-xs font-bold px-3 py-1 rounded-full border border-emerald-300">
                  {isRtl ? 'حلول مهنية 100%' : '100% Professional'}
                </div>
              </div>
            </div>
          </div>
